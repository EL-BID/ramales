...
class DockTabAbout(DockTab):
    ...
    def translate(self, msg, disambiguation=None, n=-1) -> object:
        return QCoreApplication.translate(DockTabAbout.__name__, msg, disambiguation, n)
    ...
    def tab_start_ui(self):
        ...
        lb_msg = QLabel(self.translate('SaniHUB Ramales'))
        ...
        lb_notice = QLabel(self.translate('Software livre para auxiliar no traçado e elaboração de projetos de ramais '
        ...
        lb_version = QLabel(self.translate('SaniHUB Ramales - Versão') + ' ' + version)
        lb_version.setWordWrap(True)
        gridLayoutAbout.addWidget(lb_version, 2, 0, 1, 0)

        loc = QLocale()
        lb_local = QLabel(self.translate('Local: ' + loc.nativeCountryName() +
                                  ' - Idioma: ' + loc.name()))
        gridLayoutAbout.addWidget(lb_local, 3, 0, 1, 0)
        lb_quick_guide = QLabel()
        urlLink = "<a href=\'https://www.iadb.org/'> www.iadb.org </a>"
        lb_quick_guide.setText(self.translate('Guia Rápido disponivel em ') + urlLink)
        lb_quick_guide.setOpenExternalLinks(True)
        # gridLayoutAbout.addWidget(lb_quick_guide, 4, 0, 1, 0)

        lb_financed = QLabel()
        lb_financed.setText(self.translate('O SaniHUB Ramales é um software livre que tem como objetivo auxiliar no traçado '
        ...
        lb_team = QLabel()
        lb_team.setText(self.translate('Autores'))
        lb_team.setFont(self.utils.formatBoldText())
        lb_team.setAlignment(Qt.AlignHCenter)
        gridLayoutAbout.addWidget(lb_team, 6, 0, 1, 0)

        lb_team_str = QLabel()
        lb_team_str.setText(self.translate('Analista de Conceito:') + ' Ivan Paiva' +
                            '\n' + self.translate('Desenvolvedores:') + ' Dagoberto Medeiros e Fredson Menezes' +
                            '\n' + self.translate('Cálculos e modelagem:') + 'Ivan Paiva e Flávia Rebouças')
        ...
        lb_licence_tit.setText(self.translate('Licença'))
        ...
        lb_licence = QLabel()
        lb_licence.setText(self.translate('O SaniHUB Ramales é um software Copyleft. Possui código-fonte livre para '
